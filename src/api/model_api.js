  ...
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      ...
      if (Math.abs(rankFactor) > 0.05) {
        ...
        factors.push(`Player ${betterPlayer}'s higher ranking (${Math.abs(p1Rank - p2Rank)} positions difference)`);
      }
      
      if (Math.abs(ageFactor) > 0.05) {
        const youngerPlayer = p1Age < p2Age ? 1 : 2;
        factors.push(`Player ${youngerPlayer} is younger (${Math.abs(p1Age - p2Age).toFixed(1)} years difference)`);
      }
      
      if (Math.abs(surfaceFactor) > 0.05) {
        const betterId = surfaceFactor > 0 ? 1 : 2;
        factors.push(`Player ${betterId}'s preference for ${data.surface} courts`);
      }
      
      if (Math.abs(heightFactor) > 0.05) {
        const tallerId = heightFactor > 0 ? 1 : 2;
        factors.push(`Player ${tallerId}'s height advantage (${Math.abs(heightDiff)} cm)`);
      }
      ...
      const result = {
        ...
        analysis: {
          summary: `Player ${winner} is ${confidence > 0.7 ? 'strongly' : confidence > 0.6 ? 'moderately' : 'slightly'} favored to win this match with ${(confidence * 100).toFixed(1)}% confidence.`,
          factors: factors,
...
export function formatPlayerData(data, playerNum) {
  ...
  return {
    ...
    seed: data[`${prefix}seed`] && data[`${prefix}seed`] !== 999 ? data[`${prefix}seed`] : null,
    country: getCountryFromIOC(data[`${prefix}ioc`]),
    countryCode: data[`${prefix}ioc`],
    age: data[`${prefix}age`] ? parseFloat(data[`${prefix}age`]).toFixed(1) : null,
