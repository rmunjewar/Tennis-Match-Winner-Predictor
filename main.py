...
df_balanced['match_key'] = df_balanced[['player1_ioc', 'player2_ioc', 'player1_age', 'player2_age']].astype(str).agg('-'.join, axis=1)

df_balanced = df_balanced.drop_duplicates(subset='match_key')
df_balanced = df_balanced.drop(columns=['match_key'])

print(f"Target distribution:\n{df_balanced['target'].value_counts()}")

df_balanced['age_diff'] = df_balanced['player1_age'] - df_balanced['player2_age']
df_balanced['ht_diff'] = df_balanced['player1_ht'] - df_balanced['player2_ht']
df_balanced['same_country'] = (df_balanced['player1_ioc'] == df_balanced['player2_ioc']).astype(int)
df_balanced['rank_diff'] = df_balanced['player2_rank'] - df_balanced['player1_rank']
df_balanced['avg_rank'] = (df_balanced['player1_rank'] + df_balanced['player2_rank']) / 2

# --------------------------
# Prepare Features and Labels
# --------------------------
X = df_balanced[['surface', 'tourney_level', 'age_diff', 'ht_diff', 'same_country', 'rank_diff', 'avg_rank']]
...
sns.scatterplot(x=X_pca_2d[:, 0], y=X_pca_2d[:, 1], hue=y_train, alpha=0.6)
...
print("\nFeature types:")
print("- Numeric features:", list(X.select_dtypes(include=['int64','float64'])))
print("- Categorical features:", list(X.select_dtypes(include=['object','category'])))
print("="*50 + "\n")
...
for name, model in models.items():
    ...
    if "k-NN" in name:
        model.fit(X_train_scaled, y_train)  # KNN needs scaled data
        y_pred = model.predict(X_test_scaled)
        cv_data = X_train_scaled
    else:
        model.fit(X_train, y_train)  # Trees can use raw data
        y_pred = model.predict(X_test)
        cv_data = X_train
    ...
    scores = cross_val_score(model, cv_data, y_train, cv=5)
